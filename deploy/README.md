...
- 2026-08-11 UTC：发布 Windows Agent `0.4.10`，关闭全部 Windows 弹窗通知并将 Service 错误、配对、Worker 重启、更新检查和诊断结果保留在客户端窗口内。release metadata commit 为 `dd6c11a38ce7a2ef6a356c20a0cce0448a06b483`，版本化安装包为 `AceAgentSetup-windows-amd64-V0.4.10.exe`，大小 `4917635` bytes，SHA-256 为 `2721e45d8d2b87efe6aa811c6a8be92957f808d180381710311299495a58227b`。DSM 验证时间 `2026-08-11T01:40:29Z`：`postgres`、`backend`、`web`、`updater` 均 healthy，backend/web 保持 `0.4.2`；Go 全量测试、Windows amd64 交叉编译、Windows Agent `25` 项构建契约、Windows release `11` 项发布契约、stable manifest、版本化安装包、stable alias 及公开 SHA-256/大小验证均已通过，release 容器已按 `--rm` 清理。`412-itx` 和 `ace-pc` 在发布后继续正常心跳并暂时上报 `0.4.9`，等待既有 `1 小时 + 0～10 分钟随机抖动`周期完成自动静默更新；真实 Windows 弹窗静默和窗口内反馈仍需在升级后观察。
