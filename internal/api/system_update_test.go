...
func TestSystemUpdateRoutesProxyOwnerRequests(t *testing.T) {
	...
	status := requestJSON(t, router, http.MethodGet, "/api/v1/system/update", nil, cookie)
	...
	check := requestJSON(t, router, http.MethodPost, "/api/v1/system/update/check", nil, cookie)
...
func TestSystemUpdateNilUpdaterReturnsSafeServiceUnavailableToOwner(t *testing.T) {
	...
	response := requestJSON(t, router, http.MethodGet, "/api/v1/system/update", nil, &http.Cookie{Name: sessionCookieName, Value: "authenticated-session"})
...
func requestRawSystemUpdate(handler http.Handler, body string) *httptest.ResponseRecorder {
	request := httptest.NewRequest(http.MethodPost, "/api/v1/system/update", strings.NewReader(body))
	request.Header.Set("Content-Type", "application/json")
	request.AddCookie(&http.Cookie{Name: sessionCookieName, Value: "authenticated-session"})
